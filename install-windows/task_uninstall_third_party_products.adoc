---
permalink: install-windows/task_uninstall_third_party_products.html 
sidebar: sidebar 
keywords: MS Visual C++, python, uninstall, uninstalling, 9.16 
summary: So deinstallieren Sie Produkte von Drittanbietern, einschließlich Python und Microsoft Visual C++. 
---
= Deinstallieren Sie Produkte von Drittanbietern
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Nach dem Upgrade von Unified Manager können Sie Produkte von Drittanbietern wie Python oder Microsoft Visual C++ 2013 deinstallieren. Dies ist optional.



== Deinstallieren Sie Python

.Schritte
. Wählen Sie in der *Systemsteuerung* *Programme* > *Programme und Funktionen*.
. Wählen Sie Python aus der Programmliste aus.
. Wählen Sie *Dieses Programm deinstallieren*. Deinstallieren Sie alle Python-Versionen außer 3.12.  Stellen Sie sicher, dass Sie die Python-Version 3.12 behalten.




== Deinstallieren Sie Microsoft Visual C++ 2013

.Schritte
. Wählen Sie in der *Systemsteuerung* *Programme* > *Programme und Funktionen*.
. Wählen Sie *Microsoft Visual C++ 2013* aus der Programmliste.
. Wählen Sie *Dieses Programm deinstallieren*.

