...
. Geben Sie die Nummer ein, um NFS zu konfigurieren.
...
. Um das Volume zu identifizieren, auf das der Snapshot geschrieben wird, geben Sie die IP-Adresse der Cluster-Management-Schnittstelle, den Namen der Speicher-VM, den Namen des Volumes, den LUN-Namen, den Benutzernamen und das Kennwort des Speichersystems sowie den Mount-Pfad ein.
...
. Beenden Sie die Wartungskonsole und starten Sie die Active IQ Unified Manager Schnittstelle, um einen Zeitplan für die Snapshot-Kopie zu erstellen, falls Sie dies noch nicht getan haben.
